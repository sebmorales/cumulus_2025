...
class CloudMigrationApp {
    ...
    async init() {
        ...
        // Update loading message from sentences.js if available
        if (typeof loading_msg !== 'undefined') {
            this.updateLoadingMessage(loading_msg);
        }
    ...
    displayTextAlongPath() {
        ...
        // Apply consistent minimum spacing across all screen sizes
        const minSpacing = 8;
        
        spacing = Math.max(spacing, minSpacing);
        
        // Check if text will exceed path length - truncate text rather than compress spacing below minimum
        const totalDistance = (totalChars - 1) * spacing;
        if (totalDistance > this.pathLength) {
            const maxChars = Math.floor(this.pathLength / spacing) + 1;
            if (maxChars < totalChars) {
                chars = chars.slice(0, maxChars);
                console.log(`Truncating border text to ${maxChars} characters to maintain spacing`);
            }
        }
        ...
        for (let i = 0; i < chars.length; i++) {
            const char = chars[i];
            const distance = i * spacing; // Start from beginning, not centered segments
    ...
    displayTextAlongOutline() {
        ...
        // Check if text will exceed path length - truncate text rather than compress spacing below minimum
        const totalDistance = (totalChars - 1) * spacing;
        if (totalDistance > this.outlinePathLength) {
            const maxChars = Math.floor(this.outlinePathLength / spacing) + 1;
            if (maxChars < totalChars) {
                chars = chars.slice(0, maxChars);
                console.log(`Truncating outline text to ${maxChars} characters to maintain spacing`);
            }
        }
    ...
    updateOutlineTextPositions() {
        ...
        // Check if text will exceed path length - maintain minimum spacing
        const totalDistance = (totalChars - 1) * spacing;
        let charsToUse = chars;
        if (totalDistance > this.outlinePathLength) {
            const maxChars = Math.floor(this.outlinePathLength / spacing) + 1;
            if (maxChars < totalChars) {
                charsToUse = Array.from(chars).slice(0, maxChars);
            }
        }
        
        charsToUse.forEach((char, index) => {
            const distance = index * spacing; // Same logic as displayTextAlongOutline
            const point = this.outlinePath.getPointAtLength(Math.min(distance, this.outlinePathLength));
    ...
    updateTextPositions() {
        ...
        charsToUse.forEach((char, index) => {
            const distance = index * spacing; // Same logic as displayTextAlongPath
            const point = this.borderPath.getPointAtLength(Math.min(distance, this.pathLength));
